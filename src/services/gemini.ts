 ...
 * If the fetch fails, returns a default prioritized list.
 */
export async function getSupportedModelsOrdered(apiKey: string): Promise<string[]> {
  ...
  // Use cached model list if API key hasn't changed
  ...
  try {
    ...
    if (data && Array.isArray(data.models)) {
      ...
      console.log("Supported Gemini models for this key:", supportedModels);

      const ordered: string[] = [];
      // Priority ordering
      const priorities = ['gemini-1.5-flash', 'gemini-2.0-flash', 'gemini-1.5-pro'];
    ...
    console.warn("Failed to list models, using default list:", error);
  }
  cachedModels = defaultList;
  return defaultList;
}
...
【重要な指示ルール】
1. 分析シートに表示欄がないため、AI分析のすべての文章において「リリースポイント（リリース位置の高さ・左右）」に関する言及は一切行わないでください（CSVデータに含まれていても言及は不要です）。リリースに関する言及をする場合は、分析シート上にある「リリース発射角度（縦/横）」や、そこから派生するボールの変化軌道・変化量にのみ集中してください。
2. 総合評価（summary）は、単に「クイックタイムがどう」といった特定の部分的数値に偏るのではなく、測定データ全体（球速帯、回転効率、球種ごとの変化量バランス、または長打を狙える打球速度・角度の再現性など）を多角的に網羅した、客観的かつ中身の濃い深く詳細な総括（3文程度）を記述してください。

JSONキーの説明：
- "summary": セッション全体の多角的な総括、または現在の技術的パフォーマンスの核心を突く具体的で中身の濃い総合評価（3文程度）。
...
- "strengths": 今回の測定で際立って優秀な数値、あるいは前回のセッションから特に向上した強み。
